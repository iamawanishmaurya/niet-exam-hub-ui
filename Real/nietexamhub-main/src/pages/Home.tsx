...
const Home = () => {
  ...
  const { filteredPapers, trendingPapers, loading, filters, setFilters } = useExamPapers();
  const [searchQuery, setSearchQuery] = useState("");

  // Update search filter when user types (no navigation)
  useEffect(() => {
    setFilters((prev) => ({ ...prev, searchQuery }));
  }, [searchQuery, setFilters]);

  const handleSearch = (value: string) => {
    setSearchQuery(value); // Just update search, don't navigate
  };

  const handleViewAll = () => {
    // Navigate to papers page with search query when user wants to see all
    navigate("/papers", { state: { initialSearch: searchQuery } });
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="container py-16 md:py-24">
        ...
        {/* Trending Papers - only show if not searching */}
        {!searchQuery.trim() && trendingPapers.length > 0 && (
          ...
          >
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="text-2xl font-semibold text-foreground">Trending Papers</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {/* Semester Grid - only show if not searching */}
        {!searchQuery.trim() && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
